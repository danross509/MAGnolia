...
minia_path = ""     # Enter a path for the minia installation
...
try:
    ...
    for i,arg in enumerate(sys.argv):
        ...
        # To adapt pipeline to a Nextflow conda environment
        # Added 2025-09-25, DANRoss
        elif arg == "--minia":
            minia_path = "gatb_tools/root_link" + sys.argv[i+1]
...
for ur in unpaired_reads:
    try:
        try: # some code duplication with below
            ...
            fp.close()
            fp = open(ur)
        from gatb_tools.readfq import readfq
...
def minia(k, min_abundance, prefix):
    ...
    execute(minia_path, params, memused=True)
...
def get_paired_end_parameters(contigs, library, is_mate_pairs = False):
    ...
    cmd = ["%s/gatb_tools/estimate-insert-sizes" % DIR, contigs] + paired_reads
    if is_mate_pairs:
    ...
    with open(lib_file, 'w') as f:
        for i, library in enumerate(paired_reads):
            ...
            if ' ' not in library:
                ...
                execute('gatb_tools/sga-deinterleave.pl', [library, p1, p2], interpreter="perl")
                library = p1 + " " + p2
            else:
...
if contigs is None: # need to create contigs, or are they given?
    ...
    if len(list_k_values) == 0:
        max_k = get_read_length( l_minia_reads ) #TO DO: modify bloocoo so that it gives a read length histogram, rather than using this hacky procedure to get max read length
        if max_k <= 21:
