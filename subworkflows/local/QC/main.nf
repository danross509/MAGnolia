...
workflow QC {
    ...
    BOWTIE2_BUILD_INDEX(contaminants)

    filter_contaminants(
        //trimmomatic.out,
        fastp.out.reads,
        BOWTIE2_BUILD_INDEX.out,
        bowtie2_sensitivity
        )
    ...
    if (coassembly) { 
        ...
        if (paired_reads) {
            ...
            shortreads_qc_out = concatenate_reads_1.out.join(concatenate_reads_2.out)
                .map { meta, R1, R2 ->
                    tuple(meta, [R1, R2])
                }
        } else {
            shortreads_qc_out = concatenate_reads_1.out
        }

    } else {
        shortreads_qc_out = clean_reads_ch
    }
